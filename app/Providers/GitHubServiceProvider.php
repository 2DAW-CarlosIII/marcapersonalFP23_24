...
class GitHubServiceProvider extends ServiceProvider
{
    ...
    public function pushZipFiles(Proyecto $proyecto, $rutaProyecto)
    {
        ...
        $files->each(function ($file, $key) use ($proyecto, $rutaProyecto) {
            $this->sendFile($proyecto, $file, $rutaProyecto);
        });
    ...
    public function sendFile(Proyecto $proyecto, $file, $rutaProyecto)
    {
        $repoName = $proyecto->getRepoNameFromURL();
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        $sha = $this->getShaFile($repoName, $file);
        $response = $this->client->put("/repos/{$owner}/{$repoName}/contents/{$rutaProyecto}/{$path}", [
            'json' => [
