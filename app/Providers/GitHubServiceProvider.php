...
class GitHubServiceProvider extends ServiceProvider
{
    ...
    public function pushZipFiles(Proyecto $proyecto, $rutaRepositorio)
    {
        ...
        $files->each(function ($file, $key) use ($proyecto, $rutaRepositorio) {
            $this->sendFile($proyecto, $file, $rutaRepositorio);
        });
    ...
    public function getShaFile($repoName, $file, $rutaRepositorio)
    {
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        try {
            $response = $this->client->get("/repos/{$owner}/{$repoName}/contents/{$rutaRepositorio}/{$path}");
        } catch (\Exception $e) {
        }
    ...
    public function sendFile(Proyecto $proyecto, $file, $rutaRepositorio)
    {
        $repoName = $proyecto->getRepoNameFromURL();
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        $sha = $this->getShaFile($repoName, $file, $rutaRepositorio);
        //Creamos la ruta añadiendo la ruta del ciclo / ruta del archivo
        $response = $this->client->put("/repos/{$owner}/{$repoName}/contents/{$rutaRepositorio}/{$path}", [
            'json' => [
