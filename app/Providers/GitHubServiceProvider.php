...
class GitHubServiceProvider extends ServiceProvider
{
    ...
    public function pushZipFiles(Proyecto $proyecto, $route)
    {
        ...
        $files->each(function ($file, $key) use ($proyecto, $route) {
            $this->sendFile($proyecto, $file, $route);
        });
    ...
    public function getShaFile($repoName, $file, $route)
    {
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        try {
            $response = $this->client->get("/repos/{$owner}/{$repoName}/contents/{$route}/{$path}");
        } catch (\Exception $e) {
        }
    ...
    public function sendFile(Proyecto $proyecto, $file, $route)
    {
        $repoName = $proyecto->getRepoNameFromURL();
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        $sha = $this->getShaFile($repoName, $file, $route);
        $response = $this->client->put("/repos/{$owner}/{$repoName}/contents/{$route}/{$path}", [
            'json' => [
