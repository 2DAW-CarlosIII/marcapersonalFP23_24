...
class GitHubServiceProvider extends ServiceProvider
{
    ...
    public function pushZipFiles(Proyecto $proyecto, $rutaDelFichero)
    {
        ...
        $files->each(function ($file, $key) use ($proyecto, $rutaDelFichero) {
            $this->sendFile($proyecto, $file, $rutaDelFichero);
        });
    ...
    public function getShaFile($repoName, $file,$rutaDelFichero)
    {
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        try {
            $response = $this->client->get("/repos/{$owner}/{$repoName}/contents/{$rutaDelFichero}/{$path}");
        } catch (\Exception $e) {
        }
    ...
    public function sendFile(Proyecto $proyecto, $file, $rutaDelFichero)
    {
        $repoName = $proyecto->getRepoNameFromURL();
        $owner = env('GITHUB_OWNER');
        $path = $file->getRelativePathname();
        $sha = $this->getShaFile($repoName, $file, $rutaDelFichero);
        $response = $this->client->put("/repos/{$owner}/{$repoName}/contents/{$rutaDelFichero}/{$path}", [
            'json' => [
