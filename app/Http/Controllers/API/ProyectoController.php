...
class ProyectoController extends Controller
{
    ...
    public function update(Request $request, Proyecto $proyecto)
    {
        ...
        if (isset($path) && strlen($proyecto->url_github) == 0) {
            ...
            $year = (explode("-",$proyecto->created_at))[0];
            $ciclosProyectos = $proyecto->ciclos;
            for($i = 0; $i < count($ciclosProyectos); $i++){
                $rutaProyecto = $ciclosProyectos[$i]->nombre . DIRECTORY_SEPARATOR . $year;
                $this->githubService->pushZipFiles($proyecto, $rutaProyecto);
            }
        }else{
            $proyecto->update($proyectoData);
        }
