...
class PermisosDescargasTableTest extends TestCase
{
    ...
    public function ejercicio4()
    {
        ...
        $permisoDescarga = PermisoDescarga::first();
        $curriculo = Curriculo::find($permisoDescarga->curriculo_id);
        $user = $curriculo->user;
        ...
        // Send a POST request to the endpoint
        $response = $this->put("/api/v1/curriculos/{$permisoDescarga->empresa_id}/permitirDescarga");

        // Assert that a new record has been created in the database
        $this->assertDatabaseHas('permisos_descargas', [
            'curriculo_id' => $curriculo->id,
            'empresa_id' => $permisoDescarga->empresa_id,
            'validado' => true,
        ]);
